...
final class PlayerEngine {
    ...
    private func wireUp(resolved: ResolvedPlayback, item: PlayableItem, token: Int) async {
        ...
        statusObservation = playerItem.observe(\.status, options: [.initial, .new]) { [weak self] kvoItem, _ in
            ...
            Task { @MainActor in
                ...
                await self.handleStatus(item: kvoItem, resolved: resolved, fallback: fallback)
            }
        }
    ...
    private func handleStatus(item: AVPlayerItem, resolved: ResolvedPlayback, fallback: URL?) async {
        switch item.status {
        case .readyToPlay:
        ...
        case .failed:
            ...
            if let fallback {
                ...
                statusObservation?.invalidate()
                statusObservation = nextItem.observe(\.status, options: [.initial, .new]) { [weak self] inner, _ in
                    guard let self else { return }
                    Task { @MainActor in
                        await self.handleStatus(item: inner, resolved: resolved, fallback: nil)
                    }
                }
