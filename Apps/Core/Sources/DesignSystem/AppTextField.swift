...
struct AppTextField<F: Hashable>: View {
    ...
    var body: some View {
        #if os(iOS)
        // Neither tvOS problem this type exists for applies here — a real
        // SwiftUI TextField focuses and raises the keyboard natively on
        // tap, and there's no unremovable system focus pill — so this is
        // just the field directly, wrapped in the same chrome.
        iosField
            .frame(height: 60)
        #else
        ZStack {
    ...
    @ViewBuilder private var iosField: some View {
        HStack(spacing: 10) {
            ...
            Group {
                if secure {
                    SecureField("", text: $text, prompt: Text(placeholder).foregroundStyle(Palette.text(0.32)))
                } else {
                    TextField("", text: $text, prompt: Text(placeholder).foregroundStyle(Palette.text(0.32)))
                }
            }
            .font(mono ? Mono.font(20, .semibold) : Typography.font(20, .semibold))
...
struct TVTextField: UIViewRepresentable {
    ...
    var onSubmit: () -> Void
    /// Fires when Delete is pressed while the field is already empty (the
    /// standard "nothing left to delete" UIKit signal — see
    /// `shouldChangeCharactersIn` below). Unused by plain fields;
    /// `TagSearchField` uses it to pop its most recent chip.
    ...
    final class Coordinator: NSObject, UITextFieldDelegate {
        ...
        /// Detects Delete pressed with nothing left to delete: UIKit still
        /// calls this with an empty replacement and a zero-length range at
        /// position 0 rather than just no-op'ing, which is the only signal
        /// available for "backspace on an empty field" — there's no separate
        /// delegate method for it.
        func textField(_ tf: UITextField, shouldChangeCharactersIn range: NSRange,
                       replacementString string: String) -> Bool {
            if string.isEmpty, range.length == 0, range.location == 0, (tf.text ?? "").isEmpty {
