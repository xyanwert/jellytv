...
final class ServerConnection: ObservableObject {
    ...
    init() {
        ...
        // data, no live Jellyfin needed) instead of a real handshake.
        if applyMockServerHook() { return }
    ...
    /// Dev-only hook: `JT_MOCK_SERVER` skips the real network handshake
    /// entirely and connects to an in-process mock Jellyfin server instead —
    /// a globally-registered `URLProtocol` answers the JSON/API + image
    /// requests with real TMDB/AniList-sourced data (`MockCatalogLoader`),
    /// and a tiny real loopback listener handles the two video-stream routes
    /// (AVPlayer doesn't consult app-registered `URLProtocol`s for its own
    /// media networking). Never persisted to `UserDefaults`, so it can't leak
    /// into a real reconnect later. See `MockJellyfinServer`.
    private func applyMockServerHook() -> Bool {
        guard ProcessInfo.processInfo.environment["JT_MOCK_SERVER"] != nil else { return false }
        status = .connecting("Starting mock server…")
        Task {
