...
/// The primary action, tinted by the poster: Resume with the remaining time on
/// the label and a progress bar along the button's own bottom edge, or a plain
/// Play when nothing has been watched yet.
struct OneSheetPlayButton: View {
    let label: String
    let progress: Double
    let tint: Color
    var action: () -> Void = {}
    ...
    var body: some View {
        Button(action: action) {
            ...
            // The tint is normalized to a bright accent by `DominantColor`,
            // so dark ink is the readable pairing whatever the poster is.
                ...
                if progress > 0 {
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            Rectangle().fill(Palette.page.opacity(0.22))
                            Rectangle().fill(Palette.page.opacity(0.55))
            ...
            .shadow(color: tint.opacity(0.32), radius: 22, y: 12)
        }
        .buttonStyle(FocusScaleStyle(scale: 1.04, cornerRadius: 16))
    }
}
